...
function createExpressApp(config, conversationApiFunctionModule) {
  ...
  app.use((req, res, next) => {
    ...
    const csClient = async (request) => {
      try {
        ...
        logger.info(
          { request, data: axiosResponse.data, status: axiosResponse.status },
          "CSClient reponse <-"
        );
    ...
    req.nexmo = {
      ...
      generateUserToken: (username) =>
        generateUserToken({ config, user_name: username }),
      logger,
      csClient,
      storageClient,
    };
  ...
  app.post("/rtcEvent", async (req, res) => {
    ...
    logger.info(
      { query, baseUrl, originalUrl, url, method, statusCode, body },
      "RTC Event Received <-"
    );
  ...
  app.post("/inbound", async (req, res) => {
    ...
    logger.info(
      { query, baseUrl, originalUrl, url, method, statusCode, body },
      "Messages API Inbound Event Received <-"
    );
    res.json({ body: req.body });

    const event = req.body;
    await conversationApiFunctionModule.messageEvent(event, req.nexmo);
  });
  app.post("/status", async (req, res) => {
    ...
    logger.info(
      { query, baseUrl, originalUrl, url, method, statusCode, body },
      "Messages API Status Event Received <-"
    );
    res.json({ body: req.body });
...
async function localDevSetup({ config }) {
    ...
    data: {
      ...
      capabilities: {
        ...
        messages: {
          webhooks: {
            inbound_url: {
              address: `${webhooks_url}/inbound`,
              http_method: "POST",
            },
            status_url: {
              address: `${webhooks_url}/status`,
              http_method: "POST",
            },
          },
        },
  ...
  logger.info("localDevSetup App Registration*", {
    data,
    status,
...
function checkEnvVars() {
  ...
  let mandatoryEnvs = [
    "CONV_API_FUNC_PRIVATE_KEY",
    "CONV_API_FUNC_APPLICATION_ID",
    "CONV_API_FUNC_APPLICATION_NAME",
    "CONV_API_FUNC_API_KEY",
  ];
  // if(isDev)
  mandatoryEnvs = mandatoryEnvs.concat([
    "CONV_API_FUNC_API_KEY",
    "CONV_API_FUNC_API_SECRET",
  ]);
  ...
  const emptyEnvs = mandatoryEnvs.reduce(getEmpty, []);
...
function startServer(conversationApiFunctionModule) {
  ...
  if (emptyEnvs.length) {
    ...
    return Promise.reject(
      `you need to configure the following vars`,
      emptyEnvs
    );
  }
  ...
  return Promise.resolve()
    .then(() =>
      bindLvnToApp({ phone_number, application_id, api_key, api_secret })
    )
